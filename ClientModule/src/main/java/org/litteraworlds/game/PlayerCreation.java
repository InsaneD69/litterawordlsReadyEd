...
public class PlayerCreation {
    ...
    private void getAndSetTokenID(){//получение байтов tokenID игрока с сервера,
                                    // 'O' - get "Old" tokenID, 'N' =  get "new" tokenID
        try{
            player.setTokenID(Requests.getTokenIDFromServer(player.getName(),'N'));
        }
        catch (IOException e){
            e.printStackTrace();
        }
    ...
    private String createNewUserName(){
        ...
        while(name==null){
            GameScreen.putString(TextColors.GAME_MESSAGE,"А представится все-таки стоит");
            name = inputCommand();
        }

        try{
          while(true) {
              if(!Requests.checkUniquenessNickName(name)) {

                GameScreen.putString(TextColors.GAME_MESSAGE,"Такое никнейм уже занят! Введите другой:");
                name = inputCommand();
              }
              else {
                  break;
